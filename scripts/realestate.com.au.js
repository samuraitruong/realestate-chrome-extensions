const CORS_PROXY = "https://no-cors-way.herokuapp.com/cors/";
//'https://proxy.cors.sh/'
  ...
  chrome.storage.sync.get(
    ...
    function (items) {
      ...
      const url = `https://maps.googleapis.com/maps/api/directions/json?origin=${encodeURIComponent(
        youraddress
      )}&destination=${encodeURIComponent(address)}&key=${apiKey}&mode=driving`;
      ...
      const updateAddress = (data) => {
        ...
        document
          .querySelector(".property-info__property-attributes")
          .appendChild(div);
        if (items.replaceMap) {
      ...
      if (cachedData) {
        ...
        fetch(CORS_PROXY + url, { mode: "cors" })
          .then((response) => {
  ...
  const processItem = (el) => {
    chrome.storage.sync.get(
      ...
      function (items) {
        ...
        const icon = `icon:https://s1.rui.au.reastatic.net/rui-static/img/mapping/map-pin-v2.png|${address}`;
        const mapUrl = `https://maps.google.com/maps/api/staticmap?size=640x480&maptype=roadmap&format=png&scale=1&markers=${encodeURIComponent(
          icon
        )}&key=${apiKey}&center=${encodeURIComponent(youraddress)}`;

        const updateAddress = (data) => {
          const { howFar, points } = data;
          console.log(howFar, points);
          const div = document.createElement("div");
          const originalImage = el
            .getElementsByClassName("property-image__img")[0]
            .previousSibling?.getAttribute("srcset");

          div.onmouseover = () => {
            el.getElementsByClassName("property-image__img")[0].setAttribute(
              "src",
              mapUrl
            );
            el.getElementsByClassName(
              "property-image__img"
            )[0].previousSibling.setAttribute("srcset", mapUrl);
            el.getElementsByClassName(
              "property-image__img"
            )[0].previousSibling.setAttribute("type", "image/png");
          };
          ...
          div.onmouseout = () => {
            ...
            el.getElementsByClassName(
              "property-image__img"
            )[0].previousSibling.setAttribute("srcset", originalImage);
            el.getElementsByClassName(
              "property-image__img"
            )[0].previousSibling.setAttribute("type", "image/webp");
          };
